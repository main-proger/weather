...
use crate::weather::{
    config::Config,
    info::{Date, Speed, SpeedType, Temp, TempType, WeatherInfo},
    provider::Provider,
};

static API_KEY: &str = "101b778ea4a1c56d89d54aecf4541207";
static API_CURRENT_URL: &str =
    "https://api.openweathermap.org/data/2.5/weather?q={CITY}&appid={API_KEY}";
static API_HOURLY_URL: &str =
    "https://api.openweathermap.org/data/2.5/forecast?q={CITY}&cnt={CNT}&appid={API_KEY}";
static API_DAILY_URL: &str =
    "https://api.openweathermap.org/data/2.5/forecast/daily?q={CITY}&cnt={CNT}&appid={API_KEY}";
...
impl ApiType {
    ...
    fn get_url(&self, config: &Config) -> String {
        ...
        match self {
            ApiType::Current => url,
            ApiType::Daily => url.replace("{CNT}", &config.date.as_ref().unwrap().day.to_string()),
            ApiType::Hourly => url.replace(
                "{CNT}",
                &(config.date.as_ref().unwrap().day * 8).to_string(),
            ),
        }
    }
}
...
impl JsonWeatherInfo {
    ...
    fn last_info(&self) -> &Map<String, Value> {
        self.json["list"][(self.json["cnt"].as_u64().unwrap() - 1u64) as usize]
            .as_object()
            .unwrap()
    }
}
...
impl WeatherInfo for JsonWeatherInfo {
    ...
    fn wind_deg(&self) -> Option<f64> {
        match self.api_type {
            ApiType::Current => Some(self.json["wind"]["deg"].as_f64().unwrap()),
            ApiType::Hourly => {
                let info = self.find_info_for_date();
    ...
    fn humidity(&self) -> Option<f64> {
        match self.api_type {
            ApiType::Current => Some(self.json["main"]["humidity"].as_f64().unwrap()),
            ApiType::Hourly => {
                let info = self.find_info_for_date();
    ...
    fn pressure(&self) -> Option<f64> {
        match self.api_type {
            ApiType::Current => Some(self.json["main"]["pressure"].as_f64().unwrap()),
            ApiType::Hourly => {
                let info = self.find_info_for_date();
    ...
    fn description(&self) -> Option<String> {
        match self.api_type {
            ApiType::Current => Some(
                self.json["weather"][0]["description"]
                    .as_str()
                    .unwrap()
                    .to_string(),
            ),
    ...
    fn print(&self) {
        match self.api_type {
            ...
            ApiType::Daily => {
                println!(
                    "weather in {}, on day {}:",
                    self.address().unwrap(),
                    self.config.date.as_ref().unwrap().day
                );
            }
        }
        ...
        println!(
            "  feels like temp: {}",
            self.feels_like().unwrap().to_string()
        );
